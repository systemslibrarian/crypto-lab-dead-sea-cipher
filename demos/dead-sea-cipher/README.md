...
5. **AES-256-GCM (2001)** — real Web Crypto authenticated encryption (PBKDF2-SHA256, 200k iterations, random IV/salt, 128-bit tag), a **"Peek Inside GCM" panel** that pairs the schematic (passphrase → PBKDF2 → key → counter-mode keystream → ciphertext → GHASH → tag) with a live GF(2^128) GHASH computation — H = E_K(0^128), the block chain, the mask E_K(J₀) — whose output is checked byte for byte against the tag Web Crypto produced, and a tamper control where the learner picks the ciphertext byte and bit to flip and the page reports how many of the 128 tag bits that moved, which live GCM verification then rejects.
