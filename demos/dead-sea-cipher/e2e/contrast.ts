 ...
 *    which is not the colour that lands on screen. This page leans on exactly
 *    that idiom: the revealed-key row of the OTP XOR grid, the Kasiski
 *    spotlight's dimmed characters and the crib strip's empty cells are all
 *    "de-emphasised" through `opacity` on an ancestor.
 *
 ...
 * thing as "painted underneath". The Vigenère IC meter is the local example:
 * its tick labels ("random ≈ 0.038", "English ≈ 0.065") are absolutely
 * positioned children of the gradient track, parked entirely *above* the
 * track's 6px box — the gradient is never behind them, and compositing it
 * anyway would judge them against pixels they do not sit on. So an ancestor's
 * own paint is applied only when its border box actually intersects the
 * text's box; a partial intersection still counts, so the judgement stays
 * worst-case. Opacity is unconditional either way — an opacity group fades
 * its whole subtree wherever that subtree happens to paint.
 *
 * Two more properties of this page the helper has to respect:
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The Hebrew
 *    mapping table, the Kasiski strip, the XOR grid, the crib row and the
 *    ranked-candidates list all clip their overflow, and at 380px most of
 *    their content sits outside the client box: clipped, unpainted. Its rect
 *    is still to the right of every ancestor's box, so the ancestor walk would
 *    find nothing behind it and fall through to white, reporting a ratio
 *    nothing on screen has. The Caesar chart is the same class after Break It:
 *    `overflow: hidden` plus a translateX slide pushes the leftmost bars (and
 *    their labels) out of the chart's box entirely. Skip clipped text and rely
 *    on the states/widths where the same element is visible and measured for
 *    real.
 *
 *  - TEXT PARKED OFF-SCREEN PAINTS NOTHING. The shared header's `.cl-skip-link`
 *    parks at `top: -3rem` until focused — the WCAG-sanctioned "visually
 *    hidden until focused" idiom. Measuring the parked copy invents a failure
 *    for text that is not on screen; the focused rendering is a real state and
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
    ...
    /**
     * Style and geometry are memoised per element for one pass. A loaded
     * Kasiski strip renders one span per ciphertext character and the XOR grid
     * one span per byte per row; every one of them walks the same handful of
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // Text parked off the top/left edge of the page paints no pixels — the
      // header's `.cl-skip-link` until focused. The focused rendering is a real
      // state and the gate scans it explicitly.
